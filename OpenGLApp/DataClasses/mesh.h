...
public:
  ...
  Mesh(string name, vector<Vertex> vertices, vector<unsigned int> indices,
       vector<Texture> textures, glm::vec3 materialDiffuse = glm::vec3(0.6f),
  ...
  glm::vec3 getCenter() const {
    ...
    for (const auto &v : vertices) {
      if (v.Position.x < minV.x)
        minV.x = v.Position.x;
      if (v.Position.y < minV.y)
        minV.y = v.Position.y;
      if (v.Position.z < minV.z)
        minV.z = v.Position.z;
      if (v.Position.x > maxV.x)
        maxV.x = v.Position.x;
      if (v.Position.y > maxV.y)
        maxV.y = v.Position.y;
      if (v.Position.z > maxV.z)
        maxV.z = v.Position.z;
    }
