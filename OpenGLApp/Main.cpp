...
int main() {
  ...
  //----Set Music
  menuMusic = SoundManager(getResource("Music/menu.mp3"), soundEngine.volMusic,
                           true, &soundEngine);
  sfxMouse = SoundManager(getResource("SFX/mouseClick.wav"),
                          soundEngine.volMusic, false, &soundEngine);
...
void processInput(GLFWwindow *window) {
  ...
  if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS) {
    if (startButton.selected ==
        true) { // Check if the mouse is inside the button
...
void RenderText(const Shader &shader, std::string text, float x, float y,
  ...
  for (std::string::const_iterator c = text.begin(); c != text.end(); ++c) {
    ...
    // now advance cursors for the next glyph (note that advance is the number
    // of 1/64 pixels)
    x += (ch.Advance >> 6) *
         scale; // bitshift by 6 to get value in pixels (2^6 = 64 (divide number
                // of 1/64th pixels by 64 to get number of pixels))
  }
  glBindVertexArray(0);
  glBindTexture(GL_TEXTURE_2D, 0);
}
