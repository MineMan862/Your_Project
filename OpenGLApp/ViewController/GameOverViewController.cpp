...
GameResult GameOverViewController::main(GLFWwindow *window,
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    RenderText(shaderText, "GAME OVER", centerX - 220.0f * scale,
               centerY + 100.0f * scale, 2.0f * scale,
    ...
    RenderText(shaderText, "Hai percorso " + ss.str() + " m",
               centerX - 180.0f * scale, centerY + 10.0f * scale, 0.7f * scale,
               glm::vec3(0.9f, 0.9f, 0.9f));

    // Instructions
    RenderText(shaderText, "[R] Ricomincia", centerX - 140.0f * scale,
               centerY - 80.0f * scale, 0.6f * scale,
               glm::vec3(0.7f, 0.7f, 0.7f));
    RenderText(shaderText, "[ESC] Esci", centerX - 100.0f * scale,
               centerY - 130.0f * scale, 0.6f * scale,
               glm::vec3(0.7f, 0.7f, 0.7f));
